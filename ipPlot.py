...
hdr = img1.header
dim_info = hdr['dim'] # 2D or 3D or 4D
dim_check_tmp = dim_info[3]
if dim_info[0] == 3 and dim_info[3] == 1:
    dim_check = 2
    # dimension
...
if dim_check == 2:
    ...
    x_pixdim = pix_info[1]
    y_pixdim = pix_info[2]
    max_pixdim = max(x_pixdim,y_pixdim)
    min_pixdim = min(x_pixdim,y_pixdim)
...
if dim_check == 2:
    ...
    img1_array_z = img1_array[:,vline3_up]
...
if dim_check == 2:
    ...
    # images
    im1 = ax1.imshow(img1_array[:,:].T,cmap='jet',origin='lower',vmin=cb_min, vmax=cb_max)
    ax1.plot([0, x_dim], [vline3_up, vline3_up], color='#f9fc2b', linestyle='-')
    ...
    # subplots
    ax2.plot(img1_array_z,color='#f9fc2b')
    plt.sca(ax2)
    plt.xticks([0,round(x_dim/2),x_dim], [0,round(x_dim_original/2),round(x_dim_original)], color="black")
    # plt.minorticks_on()
    plt.margins(x=0)
